...
LOCAL_RELAY = ROOT / "BridgeFlow" / "server" / "relay" / "server.py"

REMOTE_BASE = "/opt/bridgeflow"
REMOTE_SERVER = f"{REMOTE_BASE}/server.py"
REMOTE_VENV = f"{REMOTE_BASE}/venv"
REMOTE_UNIT = "/etc/systemd/system/bridgeflow-relay.service"
REMOTE_NGINX_SITE = os.environ.get("BRIDGEFLOW_NGINX_SITE", "/etc/nginx/sites-enabled/bridgeflow-site")
PUBLIC_WSS = "wss://relay.example.com/bridgeflow/ws/"

BLOCK_BEGIN = "# === BRIDGEFLOW WS BEGIN ==="
BLOCK_END = "# === BRIDGEFLOW WS END ==="
NGINX_BLOCK = dedent(
    f"""
    {BLOCK_BEGIN}
...
UNIT_TEXT = dedent(
    f"""
    [Unit]
    Description=BridgeFlow Relay
    After=network.target

    [Service]
    Type=simple
    WorkingDirectory={REMOTE_BASE}
    Environment=BRIDGEFLOW_RELAY_HOST=127.0.0.1
    Environment=BRIDGEFLOW_RELAY_PORT=5252
...
def load_ops():
    spec = importlib.util.spec_from_file_location("bridgeflow_ops", OPS_PATH)
...
def write_systemd_unit(ops) -> None:
    tmp_remote = f"{REMOTE_BASE}/bridgeflow-relay.service"
...
def patch_nginx_site(ops) -> None:
    ...
    if BLOCK_BEGIN in site_text:
        print("  [跳过] Nginx 已存在 BridgeFlow WS 配置块")
        return
    ...
    backup = f"{REMOTE_BASE}/bridgeflow-site.bak"
    tmp_remote = f"{REMOTE_BASE}/bridgeflow-site.nginx.tmp"
...
def restart_services(ops) -> None:
    ...
    ops.run_remote("systemctl enable bridgeflow-relay")
    ops.run_remote("systemctl restart bridgeflow-relay")
    ops.run_remote("systemctl status bridgeflow-relay --no-pager | head -20")
...
def verify_remote(ops) -> None:
    print("\n[verify] 远端 relay 进程")
    print(ops.run_remote("systemctl is-active bridgeflow-relay", silent=True))
    print("\n[verify] 远端 5252 监听")
    print(ops.run_remote("ss -ltnp | grep 5252", silent=True))
    print("\n[verify] Nginx 路径")
    print(ops.run_remote(f"grep -n 'bridgeflow/ws' {REMOTE_NGINX_SITE}", silent=True))
